-- ST-122: fee schedule cache (per-installment read-model) and daily reconciliation audit log.
--
...
-- ---------------------------------------------------------------------------
-- 1. fee_schedule_cache — per-installment read-model projection
-- ---------------------------------------------------------------------------

CREATE TABLE app.fee_schedule_cache (
  id                        uuid DEFAULT gen_random_uuid()
                              CONSTRAINT pk_fee_schedule_cache PRIMARY KEY,
...
CREATE UNIQUE INDEX idx_fee_schedule_cache_unique
  ON app.fee_schedule_cache (school_id, erpnext_fee_schedule_id);

-- Student installment lookup — fastest path for the student/parent UI
CREATE INDEX idx_fee_schedule_cache_student
  ON app.fee_schedule_cache (school_id, student_id, due_date ASC);

-- Overdue and status monitoring — partial index for the reconciliation job
CREATE INDEX idx_fee_schedule_cache_status
  ON app.fee_schedule_cache (school_id, status, due_date)
...
REVOKE ALL PRIVILEGES ON TABLE app.fee_schedule_cache FROM PUBLIC, studafy_app;
...
GRANT SELECT, INSERT, UPDATE ON TABLE app.fee_schedule_cache TO studafy_app;
