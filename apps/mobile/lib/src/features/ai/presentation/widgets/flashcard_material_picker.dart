...
/// Picks up to [maxSelectable] ready-to-use materials to generate a flashcard deck from — one
...
///
/// Duplicated from `QuizMaterialPicker` rather than shared, the same call `QuizCitationChip`'s doc
/// comment makes for its own duplication: the two pickers' selectable set is the same shape, but
/// each hardcodes its own feature's l10n keys (`quiz.setup.*` vs `flashcards.generate.*`), so
/// sharing one widget would mean either a generic-but-wrong label or a picker that reaches across
/// features for its copy.
class FlashcardMaterialPicker extends ConsumerWidget {
  const FlashcardMaterialPicker({
  ...
  /// Called with the toggled material's id and title — the title is captured here, not re-derived
  /// later, because there is no `GET` for "the materials a deck was generated from" once
  /// generation has happened; `FlashcardController.generateDeck` needs it purely for the deck
  /// browser's label.
  final void Function(String materialId, String materialTitle) onToggle;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (classIds.isEmpty) {
      return Padding(
        ...
        child: Text(
          'flashcards.generate.materialsUnavailable'.tr(),
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'flashcards.generate.materialCount'.tr(
            ...
            for (final material in ready)
              CheckboxListTile(
                ...
                onChanged: atLimit && !selected.contains(material.id)
                    ? null
                    : (_) => onToggle(material.id, material.title),
              ),
          ],
        );
      },
