...
/// Picks up to [maxSelectable] ready-to-use materials to generate a quiz from — one
/// [CheckboxListTile] per material, grouped by enrolled class, the same source
/// (`materialsForClassProvider`) and enrollment seam `MaterialsScreen` uses.
///
/// Only [MaterialReadyState.ready] materials are selectable: `POST .../quizzes` answers 422 for
/// one still mid-ingestion, so this keeps the setup step from ever offering a choice the server
/// would reject.
class QuizMaterialPicker extends ConsumerWidget {
  const QuizMaterialPicker({
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (classIds.isEmpty) {
      return Padding(
        ...
        child: Text(
          'quiz.setup.materialsUnavailable'.tr(),
