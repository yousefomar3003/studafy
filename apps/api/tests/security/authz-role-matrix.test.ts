 ...
 *
 * Tier A — crash/leak fuzz, generated by scanning every `createRoute()` in src/ (296 method+path
 * pairs across 226 routes as of this writing — the same set openapi.json's generator draws from,
 * scanned directly rather than reading that generated, gitignored file; see loadOperations()'s own
 * docstring for why). For each one, every role gets a real HTTP request through the real app + a
 * real Postgres. The only claims are "never 500" and "every 4xx is a well-formed problem+json
 * envelope" (errorHandler.ts's own stated contract: 5xx never leaks internals). This is honestly
 * 100% route coverage for the crash/info-leak class of bug, and nothing more — it does not know
 * what any given route's *correct* answer for a given role is.
 *
...
// ---------------------------------------------------------------------------
// Tier A — crash/leak fuzz over every operation defined in src/
// ---------------------------------------------------------------------------
...
/**
 * Every `createRoute({ method: "...", path: "..." })` operation defined anywhere in src/.
 *
 * Deliberately not read from the generated `openapi.json` (openapi:generate's output): that file
 * is gitignored, so it doesn't exist in a fresh checkout unless something ran the generator first
 * — and this suite runs directly via `bun run test:security`, with no such step ahead of it.
 * Scanning source directly makes Tier A self-contained (no build-order dependency) and, as a side
 * effect, immune to the generated spec ever drifting from the routes that actually exist.
 */
function loadOperations(): Operation[] {
  ...
  for (const file of collectSourceFiles(SRC_DIR)) {
    ...
    while ((block = createRoutePattern.exec(source)) !== null) {
      const body = block[1]!;
      const methodMatch = new RegExp(`method:\\s*["'\`](${methods.join("|")})["'\`]`).exec(body);
      const pathMatch = /path:\s*["'`](\/[^"'`]*)["'`]/.exec(body);
      if (!methodMatch || !pathMatch) continue;
      operations.push({ method: methodMatch[1]!.toUpperCase(), path: pathMatch[1]! });
    }
  }
