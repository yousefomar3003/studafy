...
`scripts/smoke-test.ts`), and `outbox-relay` and `provisioning` are still in that state.
`ai-ingestion`, `notifications`, `reports`, `imports` and `billing` carry real logic. The table below
records what each queue is _for_ and its starting concurrency.

| Queue           | `QUEUE_NAMES` key | Concurrency | Purpose                                                                                                                                                                                                                                                                                                                                                                                                              |
| --------------- | ----------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ai-ingestion`  | `AI_INGESTION`    | 2           | Ingests course/content material for AI processing: claims the material, fetches its bytes from S3, parses PDF/DOCX into heading-aware chunks with page anchors, embeds each chunk, and stores the rows on `app.material_chunks` while driving the `app.materials` ingest lifecycle. Kept low: parsing and embedding are CPU/IO-heavy and long-running. See [`parser-support-matrix.md`](./parser-support-matrix.md). |
