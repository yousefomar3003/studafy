...
describe("TesseractOcrEngine", () => {
  ...
  test("recognizes a clean printed page at >= 95 confidence with verbatim text", async () => {
  ...
  test("re-runs candidate languages and detects non-Latin script", async () => {
  ...
  test("flags a low-confidence page on the report and in the metrics", async () => {
  ...
  test("close() is idempotent and a closed engine can still recognize", async () => {
    await engine.close();
    await engine.close();
