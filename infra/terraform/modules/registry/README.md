...
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aws_ecr_repository`                             | one per `image_repository_names` entry (default 10: `api`, `realtime`, `workers`, `web`, `erpnext`, `migrations`, `prometheus`, `grafana`, `otel-collector`, `tempo`) | Named `<name_prefix>/<name>`, e.g. `studafy-prod/api`. `IMMUTABLE` tags, `scan_on_push`, SSE (`AES256`).                                                                                                  |
...
| `image_repository_names`           | `list(string)` | `["api", "realtime", "workers", "web", "erpnext", "migrations", "prometheus", "grafana", "otel-collector", "tempo"]` | One repository per entry.                                                                                                   |
