...
variable "image_repository_names" {
  description = <<-EOT
    ...
    two log-pipeline images (ST-261: infra/docker/loki.Dockerfile, vector.Dockerfile) and the
    alerting image (ST-262: infra/docker/alertmanager.Dockerfile) — all of them this repo's own
    config layered onto an upstream image, same "third-party base, our config on top" shape as
    erpnext. apps/mobile is a native app and is intentionally excluded. A list is used because the
    deployable set can grow.
  EOT
  type        = list(string)
  default     = ["api", "realtime", "workers", "web", "erpnext", "migrations", "prometheus", "grafana", "alertmanager", "otel-collector", "tempo", "loki", "vector"]
